...
export function CompanySelector() {
  ...
  // Show App Owner badge whenever the user has the App Owner role (isSuperDev),
  // regardless of which company is currently active
  ...
  if (companies.length <= 1 && !isAppOwner) {
    return (
      <div className="px-4 py-2 border-b border-[#e2e8e6] dark:border-[#2a3330]">
        <div className="flex items-center gap-2 text-sm font-medium text-[#1a2e2a] dark:text-[#e2e8e6]">
          ...
          {isAppOwner && (
            <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-gradient-to-r from-amber-500/10 to-orange-500/10 dark:from-amber-500/20 dark:to-orange-500/20 text-amber-600 dark:text-amber-400 uppercase tracking-wider inline-flex items-center gap-0.5 shrink-0">
